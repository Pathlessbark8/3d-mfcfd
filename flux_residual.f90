...
module flux_residual_mod
	...
	!
				point%flux_res(:,k) = 2.0d0*point%delt(k)*(Gxp + Gxn + Gyp + Gyn + Gzn)
	!
			enddo
	!
			do i = 1, outer_points
	...
	!
				point%flux_res(:,k) = point%delt(k) * (Gxp + Gxn + Gyp + Gyn + Gzp)
	!
			enddo
	!
	...
	!
				point%flux_res(:,k) = point%delt(k) * (Gxp + Gxn + Gyp + Gyn + Gzp + Gzn)
	!
			enddo
	!
	!
