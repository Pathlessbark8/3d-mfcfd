module implicit_UdelU_backward_sweep_outer_mod
...
!
!
!	This subroutine evaluates the UdelU of an outer boundary point during the backward sweep ..


        subroutine implicit_UdelU_backward_sweep_outer(i, UdelU)
!
!
		implicit none
...
!
		...
		nor = point%nor(:,i)
!
!	Compute dGxpos contributions to UdelU ..
!
		sum_delx_sqr = 0.00
		sum_dely_sqr = 0.00
		sum_delz_sqr = 0.00
...
!
			...
			if(alias_of_k > alias_of_i)	then
				prim = point%prim(:,k)
				call flux_Goxp(Gold, tan1, tan2, nor, prim)
				U = point%delUn(:,k) + point%U(:,k)
				call get_primitive_vector(k, U, prim)
				call flux_Goxp(Gnew, tan1, tan2, nor, prim)
				temp = Gnew - Gold
!
...
!
!
!	Compute dGxneg contributions to UdelU ..
!
		sum_delx_sqr = 0.00
		sum_dely_sqr = 0.00
		sum_delz_sqr = 0.00
...
!
			...
			if(alias_of_k > alias_of_i)	then
				prim = point%prim(:,k)
				call flux_Goxn(Gold, tan1, tan2, nor, prim)
				U = point%delUn(:,k) + point%U(:,k)
				call get_primitive_vector(k, U, prim)
				call flux_Goxn(Gnew, tan1, tan2, nor, prim)
				temp = Gnew - Gold
!
				sum_delx_delf = sum_delx_delf + temp*dels_weights
				sum_dely_delf = sum_dely_delf + temp*delt_weights
...
!
			...
			if(alias_of_k > alias_of_i)	then
				prim = point%prim(:,k)
				call flux_Goyp(Gold, tan1, tan2, nor, prim)
				U = point%delUn(:,k) + point%U(:,k)
				call get_primitive_vector(k, U, prim)
				call flux_Goyp(Gnew, tan1, tan2, nor, prim)
				temp = Gnew - Gold
!
				sum_delx_delf = sum_delx_delf + temp*dels_weights
				sum_dely_delf = sum_dely_delf + temp*delt_weights
...
!
!
!	Compute dGyneg contributions to UdelU ..
!
		sum_delx_sqr = 0.00
		sum_dely_sqr = 0.00
		sum_delz_sqr = 0.00
...
!
			...
			if(alias_of_k > alias_of_i)	then
				prim = point%prim(:,k)
				call flux_Goyn(Gold, tan1, tan2, nor, prim)
				U = point%delUn(:,k) + point%U(:,k)
				call get_primitive_vector(k, U, prim)
				call flux_Goyn(Gnew, tan1, tan2, nor, prim)
				temp = Gnew - Gold
!
				sum_delx_delf = sum_delx_delf + temp*dels_weights
				sum_dely_delf = sum_dely_delf + temp*delt_weights
...
!
!
!	Compute dGzpos contributions to UdelU ..
!
		sum_delx_sqr = 0.00
		sum_dely_sqr = 0.00
		sum_delz_sqr = 0.00
...
!
!
		UdelU = dGxp + dGxn + dGyp + dGyn + dGzp 
!
!			
        end subroutine
!
!
end module implicit_UdelU_backward_sweep_outer_mod
