...
void interior_dGz_neg(double *G, int i)
//
//
{
...
__global__ void interior_dGz_neg_multi_nccl(splitPoints *splitPoint, double power, double VL_CONST, double pi, int interiorPointsLocal, int *interiorPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector)
//
//
{
   ...
   int ind = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    //
    for (j = 0; j < splitPoint[i].numberOfLocalznegNbhs; j++)
    //
    {
        k = splitPoint[i].localzneg_conn[j];
    ...
    //
    for (j = 0; j < splitPoint[i].numberOfGhostznegNbhs; j++)
    //
    {
        k = splitPoint[i].ghostzneg_conn[j];
...
__global__ void interior_dGz_neg_multi_nccl1(splitPoints *splitPoint, double power, double VL_CONST, double pi, int interiorPointsLocal, int *interiorPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector,int multiplyFactor, bool ran)
//
//
{
