...
void interior_dGz_pos(double *G, int i)
//
//
{
	...
	for (int r = 0; r < 5; r++)
	{
		temp[r] = sum_delx_sqr*(sum_dely_sqr*sum_delz_delf[r] - sum_dely_delf[r]*sum_dely_delz) 
				- sum_delx_dely*(sum_delx_dely*sum_delz_delf[r] - sum_delx_delf[r]*sum_dely_delz) 
				+ sum_delz_delx*(sum_delx_dely*sum_dely_delf[r] - sum_delx_delf[r]*sum_dely_sqr);
	}
	//
	for (int r = 0; r < 5; r++)
	{
...
__global__ void interior_dGz_pos_cuda(points &point,int power, double VL_CONST,double pi,int interior_points,int *interior_points_index)
//
//
{
...
__global__ void interior_dGz_pos_multi_nccl(splitPoints *splitPoint, double power, double VL_CONST, double pi, int interiorPointsLocal, int *interiorPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector)
//
//
{
   ...
   int ind = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    //
    for (j = 0; j < splitPoint[i].numberOfLocalzposNbhs; j++)
    //
    {
        k = splitPoint[i].localzpos_conn[j];
    ...
    //
    for (j = 0; j < splitPoint[i].numberOfGhostzposNbhs; j++)
    //
    {
        k = splitPoint[i].ghostzpos_conn[j];
...
__global__ void interior_dGz_pos_multi_nccl1(splitPoints *splitPoint, double power, double VL_CONST, double pi, int interiorPointsLocal, int *interiorPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector,int multiplyFactor, bool ran)
//
//
{
    ...
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
	if(multiplyFactor!=6){
        if (ind < 0 || ind >= interiorPointsLocal/6)
        {
            return;
        }
    }
    else{
        if (ind < 0 || ind >= (interiorPointsLocal/6 + interiorPointsLocal%6))
        {
            return;
        }
    }
    ind=ind+(multiplyFactor-1)*interiorPointsLocal/6;
