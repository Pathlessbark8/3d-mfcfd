...
//
{
	...
	//
	{
		...
		flux_Goxn(G_i, tan1, tan2, nor, prim);
		//
		for (int r = 0; r < 5; r++)
		{
		...
		qtilde_to_primitive(qtilde, prim);
		flux_Goxn(G_k, tan1, tan2, nor, prim);
		//
		for (int r = 0; r < 5; r++)
		{
			temp[r] = G_k[r] - G_i[r];
...
//
{
	...
	//
	{
		...
		flux_Goxn_cuda(G_i, tan1, tan2, nor, prim,pi);
		//
		for (int r = 0; r < 5; r++)
		{
			temp[r] = delx * point.dq[0][r][k] + dely * point.dq[1][r][k] + delz * point.dq[2][r][k];
		...
		qtilde_to_primitive_cuda(qtilde, prim);
		flux_Goxn_cuda(G_k, tan1, tan2, nor, prim,pi);
		//
		for (int r = 0; r < 5; r++)
		{
			temp[r] = G_k[r] - G_i[r];
...
__global__ void outer_dGx_neg_multi_nccl(splitPoints *splitPoint, double power, double VL_CONST, double pi, int outerPointsLocal, int *outerPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector)
//
//
{
	int ind = blockIdx.x * blockDim.x + threadIdx.x;
