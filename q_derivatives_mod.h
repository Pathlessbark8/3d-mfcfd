...
//
{
    ...
    double sum_delx_delq[5] = {}, sum_dely_delq[5] = {}, sum_delz_delq[5] = {};
    ...
    //
    {
        ...
        for (k = 0; k < 5; k++)
        {
            point.qm[1][k][i] = point.q[k][i]; // q_maximum ..
            point.qm[2][k][i] = point.q[k][i]; // q_minimum ..
        }
        //
        ...
        //
        {
            ...
            for (k = 0; k < 5; k++)
            {
                temp[k] = (point.q[k][nbh] - point.q[k][i]);
                sum_delx_delq[k] = sum_delx_delq[k] + weights * delx * temp[k];
                sum_dely_delq[k] = sum_dely_delq[k] + weights * dely * temp[k];
                sum_delz_delq[k] = sum_delz_delq[k] + weights * delz * temp[k];
            }
            //
        }
        //
        ...
        for (k = 0; k < 5; k++)
        {
            point.dq[3][k][i] = temp[k] * one_by_det;
        }
        //
        //
    }
    ...
    //
    {
        ...
        //
        {
            ...
            //
            {
                ...
                for (r = 0; r < 5; r++)
                {
                    qtilde_i[r] = point.q[r][i] - 0.50 * (delx * point.dq[1][r][i] + dely * point.dq[2][r][i] + delz * point.dq[3][r][i]);
                    qtilde_nbh[r] = point.q[r][nbh] - 0.50 * (delx * point.dq[1][r][nbh] + dely * point.dq[2][r][nbh] + delz * point.dq[3][r][nbh]);
            ...
            for (r = 0; r < 5; r++)
            {
                point.temp[3][r][i] = temp[r] * one_by_det;
            }
            //
        } //	i loop (for points)..
          //
        for (i = 0; i < max_points; i++)
        {
            for (r = 0; r < 5; r++)
            {
                ...
                point.dq[3][r][i] = point.temp[3][r][i];
            }
        }
        //
