...
//	This subroutine evaluates the wall flux derivative dGx_neg
...
__global__ void wall_dGx_neg_cuda(points &point,double power, double VL_CONST,double pi,int wall_points,int *wall_points_index)
//
//
{
	...
	//
	{
		...
		flux_Gwxn_cuda(G_i, tan1, tan2, nor, prim,pi);
		for (int r = 0; r < 5; r++)
		{
			temp[r] = delx * point.dq[0][r][k] + dely * point.dq[1][r][k] + delz * point.dq[2][r][k];
			qtilde[r] = point.q[r][k] - 0.50 * temp[r];
...
//
//
__global__ void wall_dGx_neg_multi_nccl(int myRank,splitPoints *splitPoint, double power, double VL_CONST, double pi, int wallPointsLocal, int *wallPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector)
//
//
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    //
    {
        int nbh = splitPoint[i].localxneg_conn[j];
