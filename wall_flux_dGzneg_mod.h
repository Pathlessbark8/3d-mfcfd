...
//
//
__global__ void wall_dGz_neg_multi_nccl(splitPoints *splitPoint, double power, double VL_CONST, double pi, int wallPointsLocal, int *wallPointsLocalIndex,int* globalToLocalIndex,int **globalToGhostIndex,transferPoints **receiveBuffer,int *partVector)
//
//
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (j = 0; j < splitPoint[i].numberOfLocalNbhs; j++)
    //
    {
        k = splitPoint[i].localNbhs[j];
    ...
    //
    for (j = 0; j < splitPoint[i].numberOfGhostNbhs; j++)
    //
    {
        k = splitPoint[i].ghostNbhs[j];
